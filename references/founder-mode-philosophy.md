...
每条 numeric / factual claim 必须挂 `[PMID]` / `[NCT]` / `[NCCN-section]` / `[notebook]` anchor + SHA-256 provenance hash。G2 (PMID-quote-match) 在 write-time block 未挂 anchor 的 claim。**LLM 永远不能直接合成证据** — 这是 memory:feedback_no_offline_only 的硬执行。
...
PRD §6.5 D3 + memory:feedback_no_offline_only:**医疗 agent 不允许把"我从训练数据猜的"包装成"查到的"**。这条比任何其他原则都硬。
...
memory:feedback_no_model_downgrade:**不为省钱降 Sonnet/Haiku**。subagent fanout 用并行 (Wave 内 10 并发) 而不是降模型节省 token。这是患者侧的承诺,不是工程便利。
...
18 个 expert 都以顶级研究者的 first name 命名 (Rosa / Bert / Aviv / Iain / ...),但 `prompts/experts/<name>/persona.md` 末尾都明文 disclaimer:
...
## 4. OPL 与其他 CancerDAO Agent 的边界

| 工具 | 形态 | 阶段 | OPL 边界 |
|---|---|---|---|
| **vMTB / cancerdao-vmtb skill** | retrieval-only,一次性 board 报告 | 已确诊 + 已有 NGS | OPL = retrieval + generation + validation + memory(持续在线 PI session)。vMTB 是单次会诊;OPL 是 ongoing research team |
| **cancer-buddy** | 陪伴 / 心理 / 病历整理 / 营养 / 教育 (不做临床决策) | 任何阶段 | cancer-buddy-mind / cancer-buddy-organize 可被 Sid invoke (情绪 / 整理)。OPL 是 research-grade analysis — 每条 claim 有 PMID + provenance + 三级标签 |
| **firefly** | 罕见病诊断导航 (DeepRare HPO pipeline) | **未**确诊 / 诊断奥德赛 | firefly 解决 *toward* 诊断;OPL 解决 *from* 诊断 (已确诊后的研究 team)。诊断不明的 cancer case 走 firefly;已确诊走 OPL |
| **beacon** (已弃用并入 cancer-buddy) | — | — | — |

**OPL 不做**:
- 给未确诊患者做诊断 (firefly's job)
- 替患者做情绪 / 陪伴 / 自我管理日记 (cancer-buddy's job)
- 单次 retrieval-only 会诊 (vMTB's job)
...
**§17.3 哪些目的已实现**:
- ✅ 完整 AI scientist team (7 task-primitive + 5 domain + 10 integrator family)
