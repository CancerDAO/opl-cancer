# Integrator Catalog — 10 Family / 29 Instance
...
1. 10 Family × 29 Instance · 2. 缓存策略 (G11) · 3. 失败模式 · 4. 扩展契约 · 5. License-Constrained Sources · 6. Instances added since the core table · See also

> Count source of truth: the Discover-phase preflight registry in
> `src/opl_cancer/cli.py` enumerates **29 live integrator modules**. The table
> below details the original core instances; §6 lists the modules added since
> (taking the registry to 29). PrimeKG ships as a separate stub (not counted in
> the 29). The "10 Family" grouping is a documentation convention, not a code registry.

## 1. 10 Family × 29 Instance
...
上表是 v1-era 核心 instance;registry 现共 **29 live modules** (新增者见 §6)。所有 client 都继承 `integrators/base.py` 的 `Integrator` ABC + 走 `integrators/cache.py` 的 SQLite cache。
...
## 6. Instances added since the core table (now 29 total)

Modules added across v1.3–v2.7 that take the registry to **29** (see the
preflight list in `src/opl_cancer/cli.py`):

- **ISRCTN** (`isrctn.py`) — UK trials registry (F3); was the v1.2.0 audit I9 gap, now connected.
- **EU-CTR** (`eu_ctr.py`) + **HKCTR** (`hkctr.py`) — additional trial registries (F3).
- **EMA EAP** (`ema_eap.py`) — EU regulatory / access channel (F8).
- **BeatAML** (`beataml.py`), **Hartwig** (`hartwig.py`), **ICGC** (`icgc.py`) — genomics cohorts (F5).

### Still planned (not yet wired)
