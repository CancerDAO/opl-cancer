...
OPL for Cancer 不是把 18 个 agent 拼一起 — 它是 **双层 fractal**:外层 18 named Expert (patient mental model) × 内层 6 task-primitive grammar (engineering contract)。每个 Expert 内部跑同一套 planner→executor→reviewer→auditor→integrator→feedback,所以新增 Expert = 加 prompt 文件 + roster entry,而不是写脚本。本文件展开 PRD §2 完整架构,作为 SKILL.md 的深度阅读补充。
...
| Outer | **Expert** | 18 named (按需激活) | persona + domain + task-package portfolio + 偏好 integrators;内部 6-grammar 工作 | `src/opl_cancer/experts/<name>.py` |
...
| Outer | **Integrator pool** | 10 family ~22 instance | 全局数据源连接池;每 expert 注册"偏好数据源" | `src/opl_cancer/integrators/` |
...
## 2. 18 Expert × 5 Capability Domain 矩阵
...
```
       ...
       │ │ integrator   │ │ ←──── shared integrator pool (10 families, ~22 instances)
...
|---|---|---|---|
| **L1** | 机械门 硬规则 (20 gates,§7) | ✓ | `src/opl_cancer/validators/gates/g<N>_*.py` |
...
18 Expert 内部存在,但**不直接对患者说话**。所有 patient turn 都经 Sid:
...
理由 (ADR-0005):18 个 expert 同时跟患者说话会 cognitive overload + 把 synthesis burden 错置;trust 必须 bound 到一个 identity (Sid)。

## See also

- [`wave-lifecycle.md`](wave-lifecycle.md) — 5-Wave 单 trigger 生命周期
- [`expert-roster.md`](expert-roster.md) — 18 expert persona 完整 spec
- [`integrator-catalog.md`](integrator-catalog.md) — 22 integrator × API + cache
- [`mechanical-gates.md`](mechanical-gates.md) — L1 20 gates × failure mode
