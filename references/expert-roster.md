# Expert Roster — 18 Named Archetypes (v1.2.0+)
...
**Roster lock**:v0 锁 18 个;新增第 19 个必须 ADR + roster review (ADR-0004 followup)。

## 2. Archetype 致敬,非真人模仿 (法律安全)

所有 18 个 expert 的 `prompts/experts/<name>/persona.md` 末尾都有 `## Identity attribution` 块,明文:
...
ADR-0005 决定:**Sid 是唯一 conversational surface**。18 expert 真实存在 + 真实工作,但都不直接对患者说话。所有 patient turn 都经 Sid 编排 → expert dispatch → reviewer → Henry → Sid 合成单段 PI delivery。理由:
1. 18 channel 同时跟患者说话 = cognitive overload
2. 直接展示原始 disagreement 是把 synthesis burden 错置到患者
3. Trust 必须 bound 到一个 identity,而非分散到 18 个

## 5. Henry — IRB Substitute Auditor

`prompts/auditor/` 4 个 layer prompts:
- `l1_mechanical_gates.md` — 跑 G1-G20 全规则
...
- [`architecture.md`](architecture.md) — 18 expert × 5 domain matrix
...
- `prompts/experts/<name>/persona.md` — 18 份 persona
