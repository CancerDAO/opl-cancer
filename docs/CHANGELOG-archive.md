# Changelog — Archive (pre-v2.0 / v0.x & v1.x)

This file holds the historical changelog entries for OPL for Cancer
releases **before v2.0.0-rc1** (the v0.x scaffold series and the entire v1.x
line). They are preserved verbatim for provenance and are no longer maintained
inline in the top-level [`CHANGELOG.md`](../CHANGELOG.md). The recent v2.x major
releases live inline in that file; everything from `[1.5.7]` down lives here.
