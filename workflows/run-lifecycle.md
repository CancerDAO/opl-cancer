...
Outputs land in `triggers/<run_id>/data/` (per dataset, including `gepia3/aggregated_summary.csv`) + `meta_analysis/` (effect_sizes.csv + forest.png + funnel.png + pooled_estimates.json) + the reproducible analysis notebooks as **`triggers/<run_id>/<hyp_id>_analysis.ipynb`** (or `analysis/<hyp_id>_analysis.ipynb`).

> **Real computation, not a dry-run label (G61, BLOCK).** `wave3_data_evidence.json` records a per-analysis `mode` + a top-level `analysis_mode`. **G61** blocks delivery if Wave-3 ran dry-run-only (un-computed HR/CI/Cox/KM presented as measured), if a run is partial (some live, some dry), or if the top-level `analysis_mode` contradicts the per-run modes (a mislabel). It finds the evidence file at `run_root` or a `wave3_<ts>/` subdir. So a real Wave-3 must run live — `OPL_NATIVE_LIVE=1` (jupyter) or `OPL_BIXBENCH_LIVE=1` (docker). (Known limitation: G61 verifies the runner *executed live*, not that the notebook it ran was substantive — robustly proving computation-happened needs notebook-execution provenance, a tracked follow-up.) **Retrieval backend:** if `OPL_ONCOEVIDENCE_DB` / `ONCOEVIDENCE_DATA` points at a local OncoEvidence corpus, the Wave-1 literature step may pull PMID-anchored statements from its 375k-statement DuckDB (a fast, full-text-derived source). It is a *retrieval* aid only — deliberately NOT wired into the binding gates (G56/G2): a context-free value-binding check can only be *weakened* by adding more candidate text, so the corpus must not feed a gate haystack.
